...
func TestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input:             "(1 + 2) * 3; 4 / 2 - 5",
			expectedConstants: []interface{}{1, 2, 3, 4, 2, 5},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func TestLetStatementScopes(t *testing.T) {tests := []compilerTestCase{
	{
		input: `
		let num = 55;
		fn() { num }
...
func TestCompilerScopes(t *testing.T) {
	...
	globalSymbolTable := compiler.symTable
	...
	last := compiler.scopes[compiler.scopeIndex].lastIns
	if last.Opcode != code.OpSub {
		t.Errorf("lastInstruction.Opcode wrong. got=%d, want=%d",
			last.Opcode, code.OpSub)
	}
	if compiler.symTable.Outer != globalSymbolTable {
	...
	if compiler.symTable != globalSymbolTable {
	...
	previous := compiler.scopes[compiler.scopeIndex].prevIns
